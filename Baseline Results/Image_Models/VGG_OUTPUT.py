...
# Initialize the VGG16 model globally to avoid reloading
...
def extract_features(image_array):
    """Extract features from an image array using VGG16."""
    if image_array is not None:
        image_array = preprocess_input(image_array)
        image_array = np.expand_dims(image_array, axis=0)
        # Ensure the output is flattened correctly
        features = model.predict(image_array)
        # Flatten the output to match the expected dimensions for cosine similarity
...
def find_most_similar(features_with_path, input_features, top_k=3):
...
def main():
    ...
    with open('/Users/dhyanpatel/Desktop/IR/image-similarity/VGG_image_features.pkl', 'rb') as file:
